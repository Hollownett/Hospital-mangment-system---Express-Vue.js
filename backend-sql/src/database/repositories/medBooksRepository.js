...
class medBooksRepository extends AbstractRepository {
  constructor() {
    super();
  ...
  async create(data, options) {
    const record = await models.medBooks.create(
      {
  ...
  async update(id, data, options) {
    let record = await models.medBooks.findByPk(id, {
      transaction: AbstractRepository.getTransaction(
        options,
      ),
    });
  ...
  async findById(id, options) {
    const record = await models.medBooks.findByPk(
      id,
      {
  ...
  async _createOrUpdateFiles(record, data, options) {
    for (const field of this.fileAttributes) {
      await FileRepository.replaceRelationFiles(
        {
          belongsTo: models.medBooks.getTableName(),
  ...
  async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.patient) {
        sequelizeFilter.appendId('patientId', filter.patient);
      }
      ...
      if (filter.desiase) {
        sequelizeFilter.appendIlike('desiase', filter.desiase,'medBooks');
      }

      if (filter.recipe) {
        sequelizeFilter.appendIlike('recipe', filter.recipe,'medBooks');
      }
      if (filter.fee) {
        sequelizeFilter.appendEqual('fee', filter.fee,'medBooks');
      }
    ...
    let { rows, count } = await models.medBooks.findAndCountAll({
  ...
  async findAllAutocomplete(query, limit) {
    ...
    const records = await models.medBooks.findAll({
      attributes: ['id', 'id'],
...
module.exports = medBooksRepository;
